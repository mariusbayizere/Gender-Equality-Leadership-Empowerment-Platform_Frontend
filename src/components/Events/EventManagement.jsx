        ...
        <div className="flex-shrink-0 p-6 pb-0">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4 bg-white rounded-xl p-4 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={fetchEvents} className="p-2.5 text-white bg-blue-500 hover:bg-blue-600 rounded-xl transition-all duration-200 shadow-sm">
                <RefreshCw className="w-4 h-4" />
              </button>
          ...
          <div className="h-full bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col">
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-800 text-xl font-semibold mb-2">No events match your search</p>
                <p className="text-gray-500">Try searching with different keywords or clear the search to see all events.</p>
                <button 
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {paginatedEvents.map((event, index) => (
                        <tr key={event?.id || index} className="hover:bg-gray-50 transition-colors">
                          ...
                          {visibleColumns.event_date && (
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-700">{formatDate(event?.event_date)}</div>
                            </td>
                          )}
                          {visibleColumns.location && (
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center space-x-2">
                                <MapPin className="w-4 h-4 text-gray-400" />
                                <span className="text-sm text-gray-700">{event?.location || 'N/A'}</span>
                              </div>
                            </td>
                          )}
                          {visibleColumns.number_participants && (
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center space-x-2">
                                <Users className="w-4 h-4 text-gray-400" />
                                <span className="text-sm text-gray-700">{event?.number_participants || 'N/A'}</span>
                              </div>
                            </td>
                          )}
                          {visibleColumns.session_link && (
                            <td className="px-6 py-4 whitespace-nowrap">
                              {event?.session_link ? (
                                <a 
                                  ...
                                  target="_blank" 
                                  rel="noopener noreferrer"
                                  className="text-blue-600 hover:text-blue-800 underline truncate block max-w-xs text-sm"
                                  title={event.session_link}
                                >
                                  {event.session_link}
                                </a>
