...
import {
  MessageSquare, Trash2, User, Calendar, Tag, MoreVertical, AlertCircle, X, Plus, Search, RefreshCw, Grid3X3, Menu, Hash, Filter, Clock, Users, Zap, Code, Calendar as CalendarIcon, HelpCircle, Edit, List
} from 'lucide-react';
      ...
      <div className="h-full flex flex-col bg-gray-50">
        <div className="p-6 flex-shrink-0">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4 bg-white rounded-xl p-4 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={fetchForums} className="p-2.5 text-white bg-blue-500 hover:bg-blue-600 rounded-xl transition-all duration-200 shadow-sm">
        ...
        <div className="flex-1 flex items-center justify-center bg-white mx-6 mb-6 rounded-2xl shadow-sm border border-gray-100">
          <div className="p-12 text-center">
            ...
            <p className="text-gray-500">Try adjusting your search criteria or create your first forum post.</p>
          </div>
        </div>
      </div>
    );
            ...
            {isMobileMenuOpen && (
              <div className="mt-3 mobile-menu-container bg-gray-50 rounded-lg p-3 space-y-3 border border-gray-200">
                <div className="flex flex-wrap gap-2">
                  <button onClick={fetchForums} className="flex items-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded text-sm transition-colors">
                    <RefreshCw className="w-4 h-4" />
                    <span>Refresh</span>
                  </button>
        ...
        {filteredForums.length > 0 && (
          <div className="mb-6">
            <p className="text-gray-600 text-sm font-medium">
              Showing {actualStartIndex + 1}-{Math.min(actualStartIndex + itemsPerPage, filteredForums.length)} of {filteredForums.length} forums
            </p>
          </div>
        )}
      </div>
          ...
          <div className="h-full bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col">
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-800 text-xl font-semibold mb-2">No forums match your search</p>
                <p className="text-gray-500">Try searching with different keywords or clear the search to see all forums.</p>
                <button 
              ...
              {viewMode === 'table' ? (
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {paginatedForums.map((forum, index) => (
                        <tr key={forum?.id} className="hover:bg-gray-50 transition-colors">
                          {visibleColumns.id && (
                          ...
                          {visibleColumns.content && (
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-700">{truncateContent(forum?.content)}</div>
                            </td>
                          )}
                          {visibleColumns.category && (
                            <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          {visibleColumns.author && (
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-700">{getUserName(forum?.user_id)}</div>
                            </td>
                          )}
                          {visibleColumns.created_at && (
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-500">{formatDate(forum?.createdAt) || 'N/A'}</div>
                            </td>
                          )}
                          {visibleColumns.actions && (
                ...
                <div className="h-full overflow-auto p-6">
                  <div className="grid gap-4">
                    {paginatedForums.map((forum, index) => (
                      <MobileCard key={forum?.id} forum={forum} index={index} />
                    ))}
                  </div>
                </div>
              )}
