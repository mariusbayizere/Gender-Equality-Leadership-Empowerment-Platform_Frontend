...
const fetchCertifications = async () => {
  try {
    ...
    // If 403 Forbidden, return empty array (user doesn't have permission)
    if (response.status === 403) {
      console.warn('User does not have permission to view all certifications');
      return [];
    }
      ...
      const loadData = async () => {
        ...
        try {
          const [courseData, modulesData, enrollmentsData, certificationsData] = await Promise.all([
            ...
            fetchModules(),
            fetchEnrollments(),
            fetchCertifications() // This will now return [] instead of throwing error
          ]);
