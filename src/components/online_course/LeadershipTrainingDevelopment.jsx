...
const LeadershipTrainingDevelopment = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      return data.filter(course => course.course_type !== 'online');
    } catch (error) {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const [coursesData, modulesData, enrollmentsData, certificationsData] = await Promise.all([
          fetchCourses(),
          fetchModules(),
          fetchEnrollments(),
          fetchCertifications()
  ...
  //   const certification = getUserCertification(course.id);
  ...
  //         {/* Course Type Specific Features - Only for Webinars */}
  //         {course.course_type === 'webinar' && (
  //           <div className="mb-4">
  //             <button
  //               onClick={() => window.open(generateMeetLink(course.id), '_blank')}
  //               className="w-full flex items-center justify-center py-2 px-4 bg-green-50 hover:bg-green-100 border border-green-200 rounded-lg transition-colors"
  //             >
  ...
  //               className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white py-3 px-4 rounded-xl font-semibold transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl"
  //             >
  //               Enroll Now
  //             </button>
  //           ) : (
  ...
  // };
        ...
        <CourseCard
          key={course.id}
          course={course}
          enrollment={enrollments.find(e => e.training_course_id === course.id)}
          certification={certifications.find(c => c.training_course_id === course.id)}
          courseModules={modules.filter(m => m.training_course_id === course.id)}
          onEnroll={handleEnroll}
          onShowModules={(course) => setSelectedCourse(course)}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Enhanced Stats */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12 -mt-8 relative z-10">
          <StatsCard 
            title="Total Courses" 
